...
class MoleculeFeaturizer(AbstractFeaturizer):
    ...
    def _to_selfies(self, molecule):
        repr_kind = molecule.base_string
        rep_type = molecule.rep_type
    ...
    def get_molar_mass(self, atomic_info=None, molecule=None,):
        if atomic_info is None:
            atomic_info = self.get_element_info(
                self._to_selfies(molecule)
            )
        molar_mass = sum([TUPLE[-1] for TUPLE in atomic_info])
        return molar_mass
    ...
    def get_element_frequency(self, element, molecule=None, atomic_info=None):
        ...
        element_count = len(
            [
                element_info[element_index] for element_info in atomic_info if element_info[element_index] == element.capitalize()
            ]
        )
    ...
    def get_bond_distribution(self, molecule):
        bonds = self.get_bonds(molecule)

        return {bond: bonds.count(bond) for bond in bonds}
...
if __name__ == "__main__":
    ...
    if prob > .5:
        ...
        mols = [
            Molecule(k, v) for k, v in molecular_info.items()
        ]

        print(featurizer.featurize(mols))
        print(mols[0].base_string)

        element = "n"
        print(f"Element {element} appears {featurizer.get_element_frequency(molecule=mols[0],element=element)} times")
